...
public class SemSimOWLFactory {
	...
	public static void addClass(OWLOntology ont, Hashtable<String,String[]> urisandparenturis, OWLOntologyManager manager) {
		...
		for (int i = 0; i < keyset.length; i++) {
			String[] parentnames = urisandparenturis.get(keyset[i]);
	...
	public static Set<String> findIndexEntitiesLinkedToAnnotationComponent(OWLOntology ont, String uri) throws OWLException{
		...
		String[] relationsarray = new String[]{SemSimKBConstants.HAS_PART_URI.toString(), SemSimKBConstants.CONTAINS_URI.toString(),
				SemSimKBConstants.SEMSIM_NAMESPACE + "component_in"}; //, SemSimConstants.SEMSIM_NAMESPACE+"hasPhysicalProperty",SemSimConstants.SEMSIM_NAMESPACE+"hasComputationalComponent"};
		...
		for(String indintree : SemSimOWLFactory.traverseRelationTree(ont, uri, relationsarray)){
			String indexent = SemSimOWLFactory.getFunctionalIndObjectProperty(ont, indintree, SemSimKBConstants.SEMSIM_NAMESPACE + "isIndexEntityFor");
			if(!indexent.equals("")){
