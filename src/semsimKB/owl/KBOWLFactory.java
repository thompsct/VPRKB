...
public class KBOWLFactory {
		...
		while (scanner.hasNext()) {
			...
			if (line.contains("<owl:Ontology rdf:about=\"")) {
				if(ont.getAnnotations().isEmpty()){
					line = "<owl:Ontology rdf:about=\"http://www.virtualrat.org/VPRKB/" + ID + ".owl\"/>";
				}
				else{
					line = "<owl:Ontology rdf:about=\"http://www.virtualrat.org/VPRKB/" + ID + ".owl\">";
				}
			...
			// Replace base
				...
				line = "     xml:base=\"http://www.virtualrat.org/" + ID + ".owl\"";
			}
	...
	public static void addExternalReferenceClasses(OWLOntology destinationont,
		...
		String parentname = SemSimKBConstants.SEMSIM_NAMESPACE + "Reference_physical_" + physicaltype;
		...
		for (int i = 0; i < keyset.length; i++) {
			...
			OWLDataProperty referstoprop = factory.getOWLDataProperty(IRI.create(SemSimKBConstants.SEMSIM_NAMESPACE + "refersTo"));
	...
	public static void setIndObjectProperty(OWLOntology ont,
			Hashtable<String, String[]> table, String rel,
		...
		for (int i = 0; i < keys.length; i++) {
			if (table.get(keys[i]) != null) {
				...
				for (int y = 0; y < val.length; y++) {
					if (val[y] != null) {
						setIndObjectProperty(ont, keys[i], val[y], rel, manager);
					}
				}
			}
		}
	...
	public static void removeIndDatatypeProperty(OWLOntology ont,
			String indname, String propname, OWLOntologyManager manager) throws OWLException {
	...
	public static void removeAllIndObjectPropertyValues(OWLOntology ont, String indname, 
			String propname, OWLOntologyManager manager) throws OWLException {
		OWLIndividual ind = factory.getOWLNamedIndividual(IRI.create(indname));
		OWLObjectPropertyExpression prop = factory.getOWLObjectProperty(IRI.create(propname));
	...
	public static void removeOneIndObjectPropertyValue(OWLOntology ont,	String indname, 
			String propname, String valname, OWLOntologyManager manager) throws OWLException {
		OWLIndividual ind = factory.getOWLNamedIndividual(IRI.create(indname));
		OWLObjectPropertyExpression prop = factory.getOWLObjectProperty(IRI.create(propname));
	...
	public static Set<String> getIndDatatypeProperty(OWLOntology ont, String indname, String propname) 
			throws OWLException {
	...
	public static String getFunctionalIndDatatypeProperty(OWLOntology ont, String indname, String propname) 
			throws OWLException {
		// The hashtable keys are individual IRIs, values are relations (as IRIs)
		Set<String> values = KBOWLFactory.getIndDatatypeProperty(ont, indname, propname);
	...
	public static boolean indExistsInTree(String indtocheck, String parent, OWLOntology ontology) throws OWLException {
		boolean indpresent = false;
		Set<String> allsubclasses = KBOWLFactory.getAllSubclasses(ontology, parent, true);
	...
	public static Hashtable<String,String> getReferencedIRIs(OWLOntology ontology,String parent) throws OWLException {
		...
		Set<String> leafclasses = KBOWLFactory.getAllSubclasses(ontology, parent, true);
		for (String oneclass : leafclasses) {
			OWLClass oneowlclass = factory.getOWLClass(IRI.create(oneclass));
			for (OWLIndividual ind : oneowlclass.getIndividuals(ontology)) {
				String uri = getFunctionalIndDatatypeProperty(ontology, ind.asOWLNamedIndividual().getIRI().toString(), SemSimKBConstants.SEMSIM_NAMESPACE + "refersTo");
				String humread = getFunctionalIndDatatypeProperty(ontology, ind.asOWLNamedIndividual().getIRI().toString(), SemSimKBConstants.SEMSIM_NAMESPACE + "humanReadableName");
	...
	public static String addOntologyNickname(String ontname, String texttoappend, SemSimKBConstants constants) {
		String newtext = texttoappend;
		if (SemSimKBConstants.ONTOLOGY_NAMESPACES_AND_FULL_NAMES_MAP.containsKey(ontname)) {
			String abbrev = (String) SemSimKBConstants.ONTOLOGY_NAMESPACES_AND_FULL_NAMES_MAP.get(ontname);
	...
	public void makeURIstringFromPhysicalModelComponent(PhysicalModelComponent pmc, ModelLite semsimmodel){
		...
		if(!(pmc instanceof PhysicalProperty)){
			if(pmc.getFirstRefersToReferenceOntologyAnnotation()!=null){
				uristring = semsimmodel.getNamespace() + uristring + 
					...
					"_" + KBOWLFactory.getIRIfragment(pmc.getFirstRefersToReferenceOntologyAnnotation().getReferenceURI().toString());
			}
			else{
...
public static String[] getRDFComments(OWLOntology ont, String indiri) {
	...
	public static String getLatestVersionIDForBioPortalOntology(String bioportalID){
		...
		try {
			URL url = new URL(
					"http://rest.bioontology.org/bioportal/virtual/ontology/" + bioportalID + "?apikey=" + SemSimKBConstants.BIOPORTAL_API_KEY);
	...
	public static String getBioPortalShortIDFromURI(String uri){
		return KBOWLFactory.getIRIfragment(uri);
	}
}